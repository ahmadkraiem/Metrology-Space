...
│   │   ├── bustApexPlaneLocalization.js # Bust Apex Plane Localization Contract v0 — pure deterministic evidence-driven bust apex plane localization using orientation-normalized Side anterior prominence with gap-aware metric-Y signal processing
...
│   │   ├── directBodyMeasurements.js # Direct Body Measurements Contract v0 — pure deterministic derivation of 19 direct Batch A body measurements across Vertical Inter-Level, Projected Landmark Segments, and Kinematic Chains
...
│   │   ├── physicalMeasurementEligibility.js # Physical Measurement Eligibility Contract v0 — Dimension E consumes 4.5G only when a registered authoritative physical-geometry evaluator certifies validated/authorized/authoritative
...
│       ├── derivedMeasurementDeck.js # Right Sidebar Results cards (Cross-Section Evidence, Modeled Hip / Seat Circumference, and Direct Measurements) with click-to-highlight integration
...
│       ├── measurementHighlightOverlay2d.js # 2D Measurement Highlight Overlay renderer
...
│       ├── modeledEllipseCrossSectionPreview.js # Modeled Ellipse Cross-Section Preview SVG renderer
...
| `bodyEvidence.js` | **Body Evidence Runtime Store.** Retains active package, landmark/seg selections, derived `denseEvidenceQa` runtime state, async dense QA lifecycle integration with session race protection (`analyzeLoadedBodyEvidenceAsync`), subscriber notifications, horizontal raster slice, transverse width, profile span, cross-view correspondence, comparability QA, metric calibration provenance, physical measurement semantics, view pose semantics, clothing body-surface semantics (`getClothingBodySurfaceSemantics`, `getClothingBodySurfaceSemanticsReport`), authoritative physical evidence semantics (`getAuthoritativePhysicalEvidenceSemantics`, `getAuthoritativePhysicalEvidenceSemanticsReport`), direct body measurements getters (`getDirectBodyMeasurements`, `getDirectBodyMeasurement`), modeled cross-section perimeter getters (`getModeledCrossSectionPerimeter`, `getModeledCrossSectionPerimeterReport`), natural waist plane localization getters (`getNaturalWaistPlaneLocalization`, `getNaturalWaistPlaneLocalizationReport`), modeled natural waist circumference getters (`getModeledNaturalWaistCircumference`, `getModeledNaturalWaistCircumferenceReport`), modeled hip/seat circumference getters (`getModeledHipSeatCircumference`, `getModeledHipSeatCircumferenceReport`), bust apex plane localization getters (`getBustApexPlaneLocalization`, `getBustApexPlaneLocalizationReport`), physical measurement eligibility getters, and sanitized diagnostic export (`buildBodyEvidenceExport`). |
| `directBodyMeasurements.js` | **Direct Body Measurements Contract v0 (`direct-body-measurements-v0`).** Pure deterministic domain contract deriving 19 direct Batch A measurements from validated anatomical reference levels and promoted Front body landmarks. Evaluates Vertical Inter-Level (5), Projected Landmark Segments (10), and Kinematic Chains (4) under `valid`/`unavailable`/`invalid` status taxonomy. Strictly requires all constituent segments for chain validity; Front A-pose calibrated projected 2D distances without bilateral averaging. |
...
| `measurementSupportPolicy.js` | **Measurement Support Policy Contract v0 (`measurement-support-policy-v0`).** Pure deterministic definitions of observed supported silhouettes (`trunk_core_support_v0`, `pelvic_core_support_v0`, `trunk_pelvic_transition_support_v0`), partitioning class sets into anatomical core and clothing bridge components. |
...
| `sideAnteriorPosteriorOrientation.js` | **Side Anterior / Posterior Orientation Semantics Contract v0 (`side-anterior-posterior-orientation-v0`).** Pure deterministic domain contract determining anatomical anterior/posterior along Side-U from Side 2D pose landmarks and lateral qualification. |
...
| `modeledCrossSectionPerimeter.js` | **Modeled Cross-Section Perimeter Contract v0 (`modeled-cross-section-perimeter-v0`).** Pure deterministic domain contract deriving an ellipse-modeled cross-sectional perimeter estimate from qualified upstream cross-section evidence at the Hip Landmark Level using Ramanujan II; retained internally for QA. |
| `pelvicArbitraryYEvidenceScan.js` | **Pelvic Arbitrary-Y Evidence Scan Contract v0 (`pelvic-arbitrary-y-evidence-scan-v0`).** Pure deterministic continuous row scanner across pelvic region segmentation producing valid Front transverse width observations across candidate scan levels. |
...
| `modeledHipSeatCircumference.js` | **Modeled Hip Circumference Contract v0 (`modeled-hip-seat-circumference-v0`).** Pure deterministic domain contract deriving the primary user-facing modeled circumference estimate at the localized Maximum Seat Plane using Ramanujan II. |
...
| `modeledNaturalWaistCircumference.js` | **Modeled Natural Waist Circumference Contract v0 (`modeled-natural-waist-circumference-v0`).** Pure deterministic domain contract deriving an ellipse-modeled Natural Waist circumference estimate from the localized Natural Waist Plane candidate using Ramanujan II with a strict Front-only blocking gate. |
| `abdominalApexPlaneLocalization.js` | **Abdominal Apex Plane Localization Contract v0 (`abdominal-apex-plane-localization-v0`).** Pure deterministic evidence-driven localization of maximum anterior abdominal prominence between Natural Waist and Hip reference levels. |
| `modeledAbdominalCircumference.js` | **Modeled Abdominal Circumference Contract v0 (`modeled-abdominal-circumference-v0`).** Pure deterministic domain contract deriving an ellipse-modeled Abdominal circumference estimate from the localized Abdominal Apex Plane candidate using Ramanujan II with transition support (`trunk_pelvic_transition_support_v0`). |
| `bustApexPlaneLocalization.js` | **Bust Apex Plane Localization Contract v0 (`bust-apex-plane-localization-v0`).** Pure deterministic domain contract deriving the Bust Apex Plane localization from orientation-normalized Side anterior silhouette prominence relative to an upper-torso baseline chord between Shoulder and Natural Waist selected trough superior crest, with gap-aware metric-Y signal processing (isolated segment smoothing, interior peak detection, intra-segment broadness/support, and segmented saddle pooling). Front width and Side AP depth serve as corroboration only. |
| `measurementVisualizationProvenance.js` | **Measurement Visualization Provenance Contract v0 (`measurement-visualization-provenance-v0`).** Pure declarative domain normalizer translating domain measurement and plane localization records into standardized 2D visualization instructions across supported visualization types without recomputing measurement math. |
...
| `derivedMeasurementDeck.js` | Right Sidebar Results deck rendering collapsible Shoulder / Hip Cross-Section Evidence cards, Modeled Circumferences (Natural Waist, Abdominal, Hip), and collapsible parent Direct Measurements cards with click-to-highlight integration. |
| `measurementHighlightOverlay2d.js` | Pure declarative 2D measurement highlight overlay renderer consuming normalized visualization provenance instructions on dedicated SVG/canvas layers across Front and Side navigators. Displays clean geometry (horizontal guides, localized slice lines, endpoint dots, selection highlights) with no always-visible text labels over measurement lines. |
| `modeledEllipseCrossSectionPreview.js` | Visual-only companion cross-section preview SVG renderer for Modeled Circumferences (Natural Waist, Abdominal, Hip) displaying Front width $\times$ Side AP depth ellipse with aspect ratio preservation and disclaimer `"Ellipse model — not measured contour"`. |
