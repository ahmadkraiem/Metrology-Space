...
| `derivedMeasurementDeck.js` | Right Sidebar Results deck rendering collapsible Shoulder / Hip Cross-Section Evidence cards, Modeled Hip / Seat and Natural Waist Circumference cards, and collapsible parent Direct Measurements cards with click-to-highlight integration. |
...
| `modeledEllipseCrossSectionPreview.js` | Visual-only companion cross-section preview SVG renderer for Modeled Hip / Seat and Natural Waist Circumference displaying Front width $\times$ Side AP depth ellipse with aspect ratio preservation and disclaimer `"Ellipse model — not measured contour"`. |
